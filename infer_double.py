...
hps = utils.get_hparams_from_file("logs/double_phoneme_zh_tw/config.json")
...
# use_g_pth_file = 'G_35000' # 1220的
use_g_pth_file = 'G_250000' # 250827
_ = utils.load_checkpoint(f"logs/double_phoneme_zh_tw/{use_g_pth_file}.pth", net_g, None)
...
def synthesis(text, speaker_id, speaker_name, filename, english_flag, langauge):
    ...
    for each in str(processed_text).split(","):
        ...
        with torch.no_grad():
            ...
            o, attn, _, _ = net_g.infer(x_tst, x_tst_lengths, sid=sid, noise_scale=0.0, noise_scale_w=0.1, length_scale=1.2)
...
def synthesis_file(language, speaker_id, net_g):
    if language in ['zh']:
        ...
        test_text = language
    with open(f'gen_text/{test_text}.txt', 'r', encoding='utf8') as f:
        lines = f.readlines()
    ...
    for line in lines:
        ...
        audio_path = synthesis(text, speaker_id, "ex/" + str(speaker_id) , filename, False, language_dict[language])
        # vc(audio_path, net_g)
...
if __name__ == '__main__':
    ...
    frontend = tw_frontend(g2p_model="tw_tl")
    test_string = "tsui2-ko2 ai3-khau1-phue5 tsiah8"
    ...
    #     else:
    # test_string = ['i1', 'ie13', 'g1', 'ei13', 'u1', 'uo13', 'i1', 'ia11', 'l1', 'i14', 'a1', 'a15']
    # test_string = ['g1', 'ei13', 'n1', 'i13', 'z1', 'ii14', 'j1', 'i13', 'i1', 'ia11', 'l1', 'i14']
    ...
    result = "u1 uo13 h1 ao13 h1 h1 h1 sil h1 ai14 sil p1 a14"
    result = "zh1 ang13 d1 e15 j1 iou14 x1 iang14 c1 ang11 i1 ing12 p1 a14 p1 a14"
    # result = "x1 iong12 zh1 uang14 u1 uei11 u1 u13"
    # result = "u1 uei11 u1 u13 , u1 uei11 u1 u13 , u1 uei11 u1 u13 , u1 uei11 u1 u13 u1 uei11 u1 u13" #威武
    # result = "u1 u13 q1 i14 u1 u13 q1 i14 u1 u13 q1 i14 u1 u13 q1 i14 u1 u13 q1 i14" #武器
    # result = "u1 u13 s1 ii14 u1 u13 s1 ii14 u1 u13 s1 ii14 u1 u13 s1 ii14 u1 u13 s1 ii14" #五四

    result = "u1 uo13 i1 i12 d1 ing14 i1 iao14 i1 iong11 i1 iou13"
    result = "u1 uo13 m1 en15 i1 ie13 sh1 iii14 i1 i11 i1 iang14"
    # result = "i1 i14 zh1 iii12 z1 ai14 i1 ia11 i1 i14 a1 a15"
    result = "v1 v13 a1 a11 i1 i12 i1 i14 q1 i13 ch1 iii11 f1 an14"
    # result = "i1 i11 g1 e15 v1 van14 u1 uang14"

    print(result)
    # synthesis_file(language = "zh", speaker_id = 0, net_g = None)
    synthesis_file(language = "zh", speaker_id = 88, net_g = None)
    # synthesis_file(language = "zh", speaker_id = 86, net_g = None)
    # synthesis_file(language = "zh", speaker_id = 87, net_g = None)
    # synthesis_file(language = "zh", speaker_id = 88, net_g = None)
    # synthesis_file(language = "zh", speaker_id = 89, net_g = None)
    # synthesis(result, 15, 15, "double04.wav", False, "TW", )
