...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  net_g.train()
  net_d.train()
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers, emotions) in tqdm.tqdm(enumerate(train_loader), total=len(train_loader), desc="Training"):
    ...
    with autocast(enabled=hps.train.fp16_run):
      ...
      (z, z_p, m_p, logs_p, m_q, logs_q) = net_g(x, x_lengths, spec, spec_lengths, sid=speakers, eid=emotions)
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    with torch.no_grad():
      for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers, emotions) in enumerate(eval_loader):
        ...
        emotions = emotions[:1]
        break
      y_hat, attn, mask, *_ = generator.module.infer(x, x_lengths, sid=speakers, eid=emotions, max_len=1000)
