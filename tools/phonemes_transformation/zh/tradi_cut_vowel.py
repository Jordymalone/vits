...
class Frontend():
    ...
    # 250924 解決 c 為空導致 return 結果順序錯誤，以及 tuple 不對稱的問題
    def _get_initials_finals(self, sentence: str) -> List[List[str]]:
        ...
        for c, v in zip(orig_initials, orig_finals):
            ...
            if c in self.punc:
                initials.append(c)
                finals.append('') # 標點符號沒有對應的韻母
                continue

            # 處理正常的音節
            if c:  # 如果聲母存在 (e.g., 'h', 'tsr')
                initials.append(c + '4 ')
            else:  # 如果聲母為空 (e.g., from 'u3', 'ai2')
                initials.append('') # 加入空字串作為佔位符，確保長度一致

            # 處理韻母
            if v:
                finals.append(v[:-1] + '4' + v[-1])
            else:
                # 雖然在目前邏輯下 v 通常不為空，但加上以求完整
                finals.append('')
    ...
    # def _get_initials_finals(self, sentence: str) -> List[List[str]]:
        ...
        orig_initials, orig_finals = self._cut_vowel(ctl_text)
        for c, v in zip(orig_initials, orig_finals):
            # 當 c 為空且 v 不為空時，將 c 設為 v 的第一個字元
            # if not c and v:
            #     c = v[0]
