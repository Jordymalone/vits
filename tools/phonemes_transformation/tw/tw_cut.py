...
class tw_frontend():
    ...
    def _get_initials_finals(self, sentence: str) -> Tuple[List[List[str]], bool]:
        ...
        sentence = sentence.replace("0", "") #移除0
        ...
        for c, v in zip(orig_initials, orig_finals):
            ...
            if c and c not in self.punc:
                initials.append(c+'0')
            elif not c: # 嘗試添加母音開頭，以避免母音爭奪其他字音 2024/11/06 ＃聲母為空的字 在第一個韻母後加0
                initials.append(v[0]+'0')
            else:
                initials.append(c)
            if v not in self.punc: #非空且非標點符號 
    ...
    def get_phonemes(self, sentence: str) -> List[str]:
        ...
        # return sentence.strip(), True # just for character level training
        return r, True
